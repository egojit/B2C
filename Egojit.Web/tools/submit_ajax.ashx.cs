...
namespace Egojit.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_login(HttpContext context)
        {
            ...
            {
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "Egojit", model.user_name, 43200);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "Egojit", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "Egojit", model.user_name);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "Egojit", model.password);
            }
        ...
        private void order_save(HttpContext context)
        {
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = Egojit.Web.UI.ShopCart.GetList(userModel.group_id);
            if (iList == null)
            {
            ...
            //统计购物车
            Model.cart_total cartModel = Egojit.Web.UI.ShopCart.GetTotal(userModel.group_id);
            //保存订单=======================================================================
            ...
            //清空购物车
            Egojit.Web.UI.ShopCart.Clear("0");
            //提交成功，返回URL
