...
namespace Egojit.Web.admin.settings
{
        ...
        private bool DoAdd()
        {
            ...
            if (!string.IsNullOrEmpty(nav_title) && !string.IsNullOrEmpty(nav_url) && !string.IsNullOrEmpty(nav_sort))
            {
                try
                {
                    ...
                    List<Egojit.Model.sys_model_nav> ls = new List<Model.sys_model_nav>();
                    for (int i = 0; i < titleArr.Length; i++)
                    {
                        ls.Add(new Egojit.Model.sys_model_nav { title = titleArr[i], nav_url = urlArr[i], sort_id = int.Parse(sortArr[i].Trim()) });
                    }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (!string.IsNullOrEmpty(nav_id) && !string.IsNullOrEmpty(nav_title) && !string.IsNullOrEmpty(nav_url) && !string.IsNullOrEmpty(nav_sort))
            {
                try
                {
                    ...
                    for (int i = 0; i < titleArr.Length; i++)
                    {
                        ls.Add(new Egojit.Model.sys_model_nav { id = int.Parse(idArr[i]), model_id = model.id, title = titleArr[i], nav_url = urlArr[i], sort_id = int.Parse(sortArr[i].Trim()) });
                    }
                    model.sys_model_navs = ls;
                }
                catch
